---
title: "Supplement 1: Simulations — Inverse Probability Weights for Quasi-Continuous Ordinal Exposures with a Binary Outcome: Method Comparison and Case Study"
...
# now create plot
homoscedasticx1 <- ggplot(df[1:1000, ], aes(x = x1)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.1, alpha = 0.5, color = "grey50") +
  geom_density(adjust = 2) +
  ylab("Density") +
  scale_x_continuous(name = expression(paste(X[1], " (C: Normal; M: Normal)"))) +
...
heteroscedasticx2 <- ggplot(df, aes(x = x2)) +
  ...
  geom_density(adjust = 2) +
  ylab("Density") +
  scale_x_continuous(name = expression(paste(X[2], " (C: Normal; M: Non-Normal)"))) +
...
homoscedasticx3 <- ggplot(df, aes(x = x3)) +
  ...
  geom_density(adjust = 2) +
  ylab("Density") +
  scale_x_continuous(name = expression(paste(X[3], " (C: Non-Normal; M: Non-Normal)"))) +
  labs(caption = "  ") +
  theme + 
  theme(text = element_text(size = 8.5))
...
austin_mse <- austin_mse %>%
  ...
  mutate(name = factor(Exposure,
                       labels = c(expression(paste("A) ", X[1], " - C: Normal; M: Normal")),
                                  expression(paste("B) ", X[2], " - C: Normal; M: Non-Normal")),
                                  expression(paste("C) ", X[3], " - C: Non-Normal; M: Non-Normal")))))
